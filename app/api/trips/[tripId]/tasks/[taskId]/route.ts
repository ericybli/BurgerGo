...
export async function PATCH(req: Request, ctx: { params: Promise<{ tripId: string; taskId: string }> }) {
  const { tripId, taskId } = await ctx.params;
  return restWrite(req, async (body) => ({
    task: await updateTaskAction(taskId, body as UpdateTaskActionPatch),
  }), { tripId });
}

export async function DELETE(req: Request, ctx: { params: Promise<{ tripId: string; taskId: string }> }) {
  const { tripId, taskId } = await ctx.params;
