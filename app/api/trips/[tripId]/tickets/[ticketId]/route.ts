...
export async function PATCH(req: Request, ctx: { params: Promise<{ tripId: string; ticketId: string }> }) {
  const { tripId, ticketId } = await ctx.params;
...
export async function DELETE(req: Request, ctx: { params: Promise<{ tripId: string; ticketId: string }> }) {
  const { tripId, ticketId } = await ctx.params;
