...
export async function PATCH(req: Request, ctx: { params: Promise<{ tripId: string; expenseId: string }> }) {
...
export async function DELETE(req: Request, ctx: { params: Promise<{ tripId: string; expenseId: string }> }) {
  const { tripId, expenseId } = await ctx.params;
